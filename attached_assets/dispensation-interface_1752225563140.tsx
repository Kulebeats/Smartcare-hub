...
export default function DispensationInterface() {
  const [currentView, setCurrentView] = useState<"dispensation" | "dispenseList" | "editPrescription">("dispensation")
  const [selectedPrescription, setSelectedPrescription] = useState<Prescription | null>(null)
  const [prescriptions, setPrescriptions] = useState<Prescription[]>([
    {
      id: 1,
      drugName: "Folic acid Tablet 1 mg",
      ...
      endDate: "08-Sep-2025",
      route: "Per-Oral (PO)",
      prescribedBy: "Anthu Omwe Health Center",
      prescribedDate: "08-Jul-2025",
      dispensed: false,
    },
  ])
  ...
  const handleFinalDispense = () => {
    if (selectedPrescription) {
      // Move from prescribed to dispensed
      setPrescriptions((prev) => prev.filter((p) => p.id !== selectedPrescription.id))
      setDispensedMedications((prev) => [...prev, { ...selectedPrescription, dispensed: true }])
      setCurrentView("dispensation")
      setSelectedPrescription(null)
    }
  }

  const handleBackToDispensation = () => {
    setCurrentView("dispensation")
    setSelectedPrescription(null)
  }
                ...
                <tbody>
                  {selectedPrescription && (
                    <tr className="border-b border-gray-100">
                      <td className="py-3 px-4">{selectedPrescription.drugName}</td>
                      <td className="py-3 px-4">{selectedPrescription.frequency}</td>
                      <td className="py-3 px-4">{selectedPrescription.quantity}</td>
                      <td className="py-3 px-4">{selectedPrescription.duration}</td>
                      <td className="py-3 px-4">{selectedPrescription.route}</td>
                ...
                <tbody>
                  {prescriptions.map((prescription) => (
                    <tr key={prescription.id} className="border-b border-gray-100 hover:bg-gray-50">
                      <td className="py-2 px-3 text-sm">{prescription.drugName}</td>
                      <td className="py-2 px-3 text-sm">{prescription.frequency}</td>
                      <td className="py-2 px-3 text-sm">{prescription.quantity}</td>
                      <td className="py-2 px-3 text-sm">{prescription.duration}</td>
                      <td className="py-2 px-3 text-sm">{prescription.startDate}</td>
                      <td className="py-2 px-3 text-sm">{prescription.endDate}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
                  ...
                  {dispensedMedications.length === 0 ? (
                    ...
                    dispensedMedications.map((medication) => (
                      <tr key={medication.id} className="border-b border-gray-100">
                        ...
                        <td className="py-2 px-3 text-sm">{medication.duration}</td>
