...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const loginMutation = useMutation({
    mutationFn: async (credentials: LoginData) => {
      const res = await apiRequest("POST", "/api/smartcare/login", credentials);
    ...
    onSuccess: (user: SelectUser) => {
      ...
      /* PRODUCTION CODE (Uncommment when deploying)
      // Non-admin users get the standard redirection logic
  ...
  const registerMutation = useMutation({
    ...
    onSuccess: (user: SelectUser) => {
      ...
      toast({
        ...
        description: "Welcome to SmartCare PRO!",
      });
      
      // TEMPORARY DEV MODE: Skip facility selection for new users too
      console.log("DEVELOPMENT MODE: Bypassing facility selection for new user");
      setLocation("/patients");
      
      /* PRODUCTION CODE (Uncomment when deploying)
      // New users always need to select a facility
      setLocation("/facility-selection");
      */
    },
    onError: (error: Error) => {
  ...
  const logoutMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      // Redirect to landing page using wouter
      setLocation("/");
    },
    onError: (error: Error) => {
      toast({
